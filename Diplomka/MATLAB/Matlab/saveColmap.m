...
function write_images(path, imgs)
    ...
    for i = 1:imgs.Count
        ...
        if isfield(img,'q')
            ...
            fprintf(fid, '%g ', r2q(img.R));
        end
        fprintf(fid, '%g ', img.t);
        fprintf(fid, '%d %s\n', img.camera_id, strtrim(img.name));
        
        for j = 1:size(img.xys,1)
            fprintf(fid, '%g %g %d ',img.xys(j,1), img.xys(j,2), img.point3D_ids(j));
        end
...
function write_points3D( path, points3D )
    ...
    for i = 1:points3D.Count
        ...
        fprintf(fid, '%g ', pt.xyz);
        fprintf(fid, '%d ', pt.rgb);
        fprintf(fid, '%g ', pt.error);
        track_ids = pt.track - repmat(int64([0 1]),size(pt.track,1),1);
        fprintf(fid, '%g ', track_ids');
        fprintf(fid, '\n');
    end
    fclose(fid);
end
